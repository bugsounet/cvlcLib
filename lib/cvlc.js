...
Cvlc.setMethod(function getInstance(callback) {
  ...
  this.hinder = Fn.hinder(function initVlc(done) {
    ...
    Fn.series(function getPort(next) {
      ...
      var args = []
      if (that.options) { /** execute array of options **/
        args = that.options
        args.unshift('--intf', 'telnet', '--telnet-port', that.port, '--telnet-password', that.password, '-')
      } else { /** default Youtube Player **/
        args = [
          "--fullscreen", "--play-and-exit", '--video-on-top', "--no-video-deco", "--no-embedded-video", "--video-title=YouTube",
          "--no-http-forward-cookies", // enforce Youtube playing !!!
          '--intf', 'telnet',
 ...
 */
Cvlc.setMethod(function play(optional_file, callback, afterPlaying) {

  if (afterPlaying) afterClosed= afterPlaying
  var that = this;
